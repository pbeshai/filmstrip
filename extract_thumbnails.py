...
def writeImagePyramid(destPath, name, seqNumber, image):
    ...
    eighthPath = os.path.join(destPath, "eighth", name + "-" + str(seqNumber) + ".png")
...
#
def extractThumbnails(sourcePath, destPath, data, name, verbose=False):
    ...
    # numThumbnails = 30
    # frameInterval = numFrames / numThumbnails
    # print(numFrames, numThumbnails, frameInterval)
...
def makeOutputDirs(path):
    try:
        ...
        os.makedirs(os.path.join(path, "images", "eighth"))
...
data = extractThumbnails(args.source, args.dest, data, args.name, args.verbose)
